...
<template>
  <div>
    ...
    <u-card :title="goods_name" :sub-title="bay_at" :thumb='goods_cover' title-size="38">
      ...
      <view class="" slot="foot">
        <navigator class="u-flex u-light-color" :url="'/pages/comment/index?goods_id=' + goods_id" hover-class="navigator-hover">
          <u-icon name="chat-fill" size="34" color="" :label="comment_number + '评论'"></u-icon>
        </navigator>
      </view>
    </u-card>
    ...
    <u-card class="comment-wrap" title="评论">
      <view slot="body">
        ...
        <u-gap height="15"></u-gap>
        <u-cell-group>
          <u-field v-model="commentContent" label="评论内容" placeholder="请输入您想要说的话" 
          type="textarea" maxlength="128" :field-style="{height: '80px'}">
          </u-field>
        </u-cell-group>
  ...
  methods: {
    ...
    getDetail(code){
      ...
      getTicketByCode(code).then((response) => {
        ...
        this.comment_number = response.data.comment_number
