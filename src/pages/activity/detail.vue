<template>
    <view>
        ...
        <!--			<swiper-item v-for="(item,index) in [activity.cover]" :key="index">-->
        ...
        <!--		<view class="info">-->
        <!--			<view class="title">-->
        <!--				<text :class="{Skeleton:!loaded}">{{activity.title}}</text>-->
        <!--				<text :class="{Skeleton:!loaded}">{{activity.desc}}</text>-->
        <!--			</view>-->
        <!--		</view>-->
        <!--        -->
        <view class="body">
        ...
        <view class="actions">
            ...
            <text class="yticon icon-shoucang" :class="{active: favorite}" @click="toFavorite"></text>
        </view>
...
<script>
    ...
    export default {
        ...
        async onLoad(params) {
            this.activity_id = parseInt(params.id)
            ...
            uni.setNavigationBarTitle({title: this.activity.title})
            let collectRes = await isCollect('activity', this.activity_id)
        ...
        methods: {
            ...
            toFavorite() {
                ...
                if (flag) {
                    this.$api.user.addUserCollection('activity', this.activity.id).then(res => {
                        if (res.code == 200) {
                            this.favorite = flag
                        }
                    })
                } else {
                    this.$api.user.delUserCollection('activity', this.activity.id).then(res => {
                        if (res.code == 200) {
                            this.favorite = flag
                        }
                    })
            ...
            getDetail() {
                this.$api.site.getActivityDetail({id: this.activity_id}).then((response) => {
