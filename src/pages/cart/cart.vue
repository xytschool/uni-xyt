<template>
  <view class="container">
    ...
    <view v-if="!hasLogin || empty === true" class="empty">
      <image
        src="http://data.xytschool.com/m/v1/static/emptyCart.jpg"
        mode="aspectFit"
      ></image>
        ...
        <block v-for="(item, index) in cartList" :key="index">
          ...
          >
            ...
            <view class="item-right">
              ...
              <text class="price"
                >¥{{ item.real_price }}
                <text class="origin_price">¥{{ item.price }}</text></text
              >
              <uni-number-box
                class="step"
                :min="1"
...
export default {
  ...
  methods: {
    ...
    //创建订单
    createOrder() {
      console.log(this.cartList, '345678765434567898765435678765456')
      // let list = this.cartList
      // let goodsData = []
      // list.forEach((item) => {
      //   if (item.checked) {
      ...
      // this.$store.dispatch('order/preOrderBygoodsType', this.cartList)
